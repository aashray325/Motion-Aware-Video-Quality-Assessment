# # stage_c.py
# # Stage C: Wighted Spatial Pooling Calculation
...
#     def __init__(self, window_size=7, debug_video_name="debug", debug=True,
#                  pooling_mode="localized"):
...
#             dist_frame = cv2.resize(dist_frame, (w, h))

#         ref_frame = ref_frame.astype(np.uint8)
#         dist_frame = dist_frame.astype(np.uint8)
...
#         if self.POOLING_MODE == "localized":
#             if np.sum(msa_enhanced) < 1e-12:
#                 mean_error = np.mean(error_map)
#                 final_quality = 1.0 - mean_error
#                 mean_salient_error = mean_error
#                 salient_mask_sum = 0
#             else:
...
#                     thresh = np.percentile(msa_enhanced, max(50.0, self.TOP_PERCENTILE - 20.0))
#                     salient_mask = (msa_enhanced >= thresh).astype(np.float32)

#                 weighted_error_salient = error_map * (1.0 + self.SALIENCY_PENALTY * msa_enhanced) * salient_mask
#                 salient_mask_sum = np.sum(salient_mask)

#                 if salient_mask_sum < 1e-8:
#                     mean_salient_error = np.mean(error_map)
#                 else:
#                     mean_salient_error = np.sum(weighted_error_salient) / (salient_mask_sum + 1e-8)

#                 mean_error_global = np.mean(error_map)
#                 alpha = 0.75 
#                 mean_error = alpha * mean_salient_error + (1.0 - alpha) * mean_error_global
#                 final_quality = 1.0 - mean_error

#         else:
#             weight_map = 1.0 + (msa_enhanced * self.SALIENCY_PENALTY)
#             weighted_error_map = error_map * weight_map
#             sum_werr = np.sum(weighted_error_map)
#             sum_w = np.sum(weight_map)
#             mean_error = np.mean(error_map) if sum_w < 1e-8 else sum_werr / (sum_w + 1e-8)
#             final_quality = 1.0 - mean_error

#             salient_mask_sum = np.sum(msa_enhanced > 0.001)
#             mean_salient_error = np.sum(error_map * (msa_enhanced > 0.001)) / (salient_mask_sum + 1e-8)
...
# stage_c.py
# Stage C: Weighted Spatial Pooling Calculation (cleaned + compact)
